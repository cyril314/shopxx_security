...
public class DefaultSecurityMetadataSource extends JdbcDaoSupport implements FilterInvocationSecurityMetadataSource {
    ...
    private String sql = "SELECT a.`VALUE` FROM sys_resource a, sys_role_resource b, sys_role c where a.ID = b.`resource_id` and b.ROLE_ID = c.id and c.NAME=?";
    ...
    public DefaultSecurityMetadataSource(DataSource dataSource) {
        ...
        List<String> query = getJdbcTemplate().query("select sas.`VALUE` from sys_role sas", new RowMapper<String>() {
