...
<body class="index">
	...
	<div class="body">
		...
		<div class="newProduct">
			...
			<div class="right">
				<#list rootProductCategoryList as pcs>
					<ul class="newProductTabContent">
                        <#if newProductMap[pcs.id]??>
						<#list newProductMap[pcs.id] as list>
                            <li>
                                <a href="${base!}${list.htmlFilePath!}">
                                    <img src="${base!}${(list.productImageList[0].thumbnailProductImagePath)!systemConfig.defaultThumbnailProductImagePath}" alt="${list.name}" />
                                    <#if (list.name?length < 12)>
					...
					<ul>
						<#list (hotArticleList)! as list>
							<li class="number${list_index + 1}">
                                ...
                                <a href="${base!}${list.htmlFilePath!}" title="${list.title}">${list.title?substring(0, [12, (list.title!"")?length]?min)}...</a>
							</li>
							<#if list_index + 1 == 10>
								<#break/>
							</#if>
