...
<div class="header png">
        ...
        <div class="headerTopContent">
            ...
            <div class="headerTopNav">
                <#list topNavigationList as list>
                    <a href="${list.url }"<#if list.isBlankTarget == true> target="_blank"</#if>>${list.name}</a>
                    <#if list_has_next>|</#if>
                </#list>
            </div>
        </div>
    ...
    <div class="headerMiddle">
        <div class="headerInfo"> 7×24小时服务热线：<strong>${(systemConfig.phone)!}</strong></div>
            ...
            <form id="productSearchForm" action="${base!}/shop/product!search.action" method="get">
                <div class="headerSearchText">
                    <input type="text" id="productSearchKeyword" name="pager.keyword"
                           value="<#if (pager?? && pager.keyword?? && rootProductCategoryList??)>${pager.keyword}<#else>请输入关键词...</#if>"/>
                </div>
                <input type="submit" class="headerSearchButton" value=""/>
                <div class="hotKeyword">
                    热门关键词:
                    <#list systemConfig.hotSearchList as list>
                        <a href="${base!}/shop/product!search.action?pager.keyword=${list}">${list}</a>
                    </#list>
                </div>
            </form>
        </div>
    ...
    <div class="headerBottom">
        <input type="button" class="cartItemListButton showCartItemList" onclick="window.open('${base!}/shop/cart_item!list.action')"/>
        <ul id="cartItemListDetail"></ul>
        <input type="button" class="orderButton" onclick="window.open('${base!}/shop/cart_item!list.action')"/>
        <div class="headerMiddleNav">
            ...
            <ul class="headerMiddleNavContent png">
                <#list middleNavigationList as list>
                    <li><a href="${list.url}"<#if list.isBlankTarget == true> target="_blank"</#if>>${list.name}</a></li>
                </#list>
            </ul>
