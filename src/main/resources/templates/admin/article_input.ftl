    ...
    <form id="inputForm" class="validate" action="<#if isAdd??>article!save.action<#else>article!update.action</#if>" method="post">
        ...
        <table class="inputTable">
                ...
                <td>
                    <select name="article.articleCategory.id" class="{required: true}">
                        ...
                        <#list articleCategoryTreeList as list>
                            <option value="${list.id}"<#if (list.id == (article.articleCategoryId)!)!> selected</#if>>
                                <#if list.level != 0>
            ...
            <tr>
                <th> 是否推荐:</th>
                <td>
                    <label><input type="radio" name="article.isRecommend" value="true"<#if (article.isRecommend)!false> checked</#if>
                        />是</label>
                    <label><input type="radio" name="article.isRecommend" value="false"<#if (isAdd || !(article.isRecommend)!false)>
                    checked</#if> />否</label>
                </td>
            </tr>
            <tr>
                <th> 是否置顶:</th>
                <td>
                    <label><input type="radio" name="article.isTop" value="true"<#if (article.isTop)!false> checked</#if> />是</label>
            ...
            <tr>
                <th> 是否发布:</th>
                <td>
                    <label><input type="radio" name="article.isPublication" value="true"
                                <#if (isAdd || article.isPublication)!false> checked</#if> />是</label>
                    <label><input type="radio" name="article.isPublication" value="false"
                                <#if !(article.isPublication)!false> checked</#if> />否</label>
                </td>
            </tr>
            <tr>
                <th> 内容:</th>
                <td>
                    <textarea name="article.content" class="wysiwyg {required: true, messagePosition: '#contentMessagePosition'}" rows="20"
                              cols="100">${(article.content)!}</textarea>
                    <div class="blank"></div>
