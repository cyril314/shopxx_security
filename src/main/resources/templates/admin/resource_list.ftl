    ...
    <form id="listForm" action="resource!list.action" method="post">
        <div class="operateBar">
            ...
            <select name="pager.property">
                <option value="name" <#if pager.property! == "name">selected="selected" </#if>> 资源名称</option>
                <option value="value" <#if pager.property! == "value">selected="selected" </#if>> 资源值</option>
            </select>
        ...
        <table class="listTable">
            ...
            <#list pager.list as list>
                <tr>
                    <td>
                        <input type="checkbox" <#if list.iSys??>disabled title="系统内置资源不允许删除!" <#else>name="ids" value="${list.id}"</#if>/>
                    </td>
                    <td>
                        ${list.name}
                    </td>
